...
class MultiObjectiveParticleSwarmOptimization:
    ...
    def __init__(self, weight=0.7, learning_rates=(1.5, 1.5), max_gen=3000, 
        ...
        self.objectives = objectives if objectives is not None else [self._objective1, self._objective2]
    ...
    def plot_state(self, title):
        ...
        参数:
            title: 字符串，图形标题
        """
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 6), gridspec_kw={'width_ratios': [1, 1]})
        ...
        # 左图：粒子适应度分布
        ax1.set_xlabel('Objective 1 Fitness', fontsize=12, fontweight='bold')
        ax1.set_ylabel('Objective 2 Fitness', fontsize=12, fontweight='bold')
        ax1.set_title(f'{title} - 粒子适应度分布', fontsize=14, fontweight='bold')
        ...
        # 绘制所有粒子的适应度
        sc = ax1.scatter(self.pop_fitness[:, 0], self.pop_fitness[:, 1], c=np.sum(self.pop_fitness, axis=1), 
                         cmap=cmap, norm=norm, alpha=0.6, s=50, edgecolors='k', label='粒子')
        
        # 突出显示全局最优位置
        ax1.plot(self.pop_gbest_fitness[0], self.pop_gbest_fitness[1], 'ro', markersize=10, 
                 label='全局最优')
        
        # 添加色条
        cbar = plt.colorbar(sc, ax=ax1)
        cbar.set_label('Total Fitness', fontsize=12)
        ...
        ax2.set_title(f'{title} - 最佳适应度', fontsize=14, fontweight='bold')
...
if __name__ == "__main__":
    ...
    pso = MultiObjectiveParticleSwarmOptimization(
        objectives=[lambda x: np.sum(x**2), lambda x: np.sum((x - 1)**2)],
        fitness_functions=[fitness1, fitness2]
    )
