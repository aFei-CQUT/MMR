...
class ParticleSwarmOptimization:
    ...
    def _init_pop_v_fit(self):
        ...
        返回:
            pop_positions: ndarray,种群位置,形状为 (pop_size, 2)
    ...
    def _get_init_best(self):
        ...
        返回:
            pop_gbest: ndarray,全局最优位置,形状为 (2,)
            pop_gbest_fitness: 浮动,全局最优适应度
            pop_pbest: ndarray,个体最优位置,形状为 (pop_size, 2)
    ...
    def _update_velocities(self, r1, r2):
        ...
        参数:
            r1: ndarray,随机数,形状为 (pop_size, 2)
            r2: ndarray,随机数,形状为 (pop_size, 2)
        """
    ...
    def plot_state(self, title):
        ...
        # 绘制所有粒子
        sc = ax1.scatter(self.pop_positions[:, 0], self.pop_positions[:, 1], c=self.pop_fitness, 
                         cmap=cmap, norm=norm, alpha=0.6, s=50, edgecolors='k', label='粒子')
        
        # 突出显示全局最优位置
        ax1.plot(self.pop_gbest[0], self.pop_gbest[1], 'ro', markersize=20, 
                 label='全局最优')
        
        # 添加色条
        cbar = plt.colorbar(sc, ax=ax1)
        cbar.set_label('适应度', fontsize=12)
        
        ax1.legend()
        
        # 右图：最佳适应度
        ax2.set_xlabel('迭代次数', fontsize=12, fontweight='bold')
        ax2.set_ylabel('最佳适应度', fontsize=12, fontweight='bold')
        ax2.set_title(f'{title} - 最佳适应度', fontsize=14, fontweight='bold')
        
        if title == "初始状态":
            ax2.plot([0], [self.pop_gbest_fitness], 'ro', markersize=8)
        else:
    ...
    def update(self):
        ...
        for i in range(1, self.max_gen):  # 从1开始,因为0已经用于初始状态
            r1, r2 = np.random.rand(2, self.pop_size, 2)
