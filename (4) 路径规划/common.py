...
class GridMap:
    ...
    def show_path(self, path_list, *, save = False):
        ...
        # map_ = cv2.cvtColor(map_, cv2.COLOR_BGR2RGB)        # 如果需要RGB显示
        map_ = map_[::-1]                                     # 反转y轴方向以正确显示地图
...
def toc(name='', *, CN=True, digit=6): 
    '''计时结束'''
    if 'global_tic_time' not in globals() or not global_tic_time: # 未设置全局变量或全局变量为空列表
...
def limit_angle(x, mode=1):
    ...
    float
        归一化后的角度值
    """
    x = x - x // (2 * math.pi) * 2 * math.pi    # 归一化到 [0, 2π)
